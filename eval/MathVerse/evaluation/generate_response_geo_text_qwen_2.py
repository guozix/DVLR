...
from vllm_qwenvl import init_qwen_2_5, chat_qwen_2_5
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_question', action='store_true', help='?')
    ...
    from llm_api import make_msg_wimage
    ...
    for _, pid in enumerate(tqdm(test_pids)):
        ...
        if args.use_familiar_hint:
            # query = "First perform reasoning, then finally select the question from the choices in the following format: Answer: xxx." + query
            if args.add_question:
                query = "First perform reasoning, then finally answer in the following format: Answer: xxx.\n" + problem["question"] + "\n" + query
            else:
                query = "First perform reasoning, then finally answer in the following format: Answer: xxx.\n" + query
        else:
            query = query

        text_image_input = make_msg_wimage([query])

        print(query)
        print(f"\nGenerating response for {pid}...")
        response = chat_qwen_2_5(text_image_input)
