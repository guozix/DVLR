...
def normalize_extracted_answer(extraction, question_type):
    """
...
if __name__ == '__main__':
    ...
    for pid in full_pids:
        ...
        # normalize the extracted answer to match the answer type
        prediction = normalize_extracted_answer(extraction, question_type)
    ...
    target_keys = ['question_type', 'source', 'problem_version']
    ...
    for key in target_keys:
        ...
        if key == 'problem_version':
            scores[key] = dict(sorted(scores[key].items(), key=lambda item: sort_sets[item[0]]))
        else:
